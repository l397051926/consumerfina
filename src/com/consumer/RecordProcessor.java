...
public class RecordProcessor implements Runnable{
        ...
        try {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (record != null) {
                    ...
                    //处理过程
                    if(KafkaMultiProcessorTest.kai){
                        new Thread(new Statistics()).start();
                    }
                    KafkaMultiProcessorTest.kai=false;
                    KafkaMultiProcessorTest.num++;

                    process(record);

                    //完成任务数加1
                    System.out.println("---------sum:   "+KafkaMultiProcessorTest.num);
    ...
    private void process(ConsumerRecord<String, String> record) {
       ...
       // System.out.println("topic: "+record.topic()+"partition: "+record.partition()+"offset: "+record.offset()+"valuesize: "+record.serializedValueSize());
